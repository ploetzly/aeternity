...
reward_txs(Block, aec_consensus_hc) ->
    %% Get stakeholder from block
    %% Call reward contract to find out about a possible pin reward
    %% (amount is part of contract)
    Delay = aec_governance:beneficiary_reward_delay(), %% most likely configured 0
    Node = aec_chain_state:wrap_block(Block),
    Trees = aec_chain_state:grant_hc_fees(Node, aec_trees:new(), Delay, false, nil),
    Amount = 0, %% We don't take money out of the contract to pay awards??
    %% aec_consensus_hc:state_grant_reward(Beneficiary, Node, Trees, Amount),
    [];
